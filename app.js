...
function render() {
  ...
  const preserveCanvas = activeFieldCanvas && html.includes("data-node-field");
  const reusedCanvas = preserveCanvas ? activeFieldCanvas : null;
  if (!preserveCanvas) destroyField();

  app.innerHTML = html;

  if (reusedCanvas) {
    const placeholder = app.querySelector("canvas[data-node-field]");
    if (placeholder) placeholder.replaceWith(reusedCanvas);
  }
  paintAvatars(app);
  if (!reusedCanvas) {
    activeField = initNodeField(app);
    activeFieldCanvas = activeField ? activeField.canvas : null;
  }
  ...
  if (ONLINE) {
    ...
    try {
      await api.sendFeedback(state.token, {
        ...
        currentGame: state.need?.game || null,
