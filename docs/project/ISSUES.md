...
- **Latest stateful verification:** `capstate500-terminalized-20260825` 复用 5 个普通 synthetic accounts，runner stage 5（`2 Ranked + 3 Casual`）PASS；125 条 lifecycle entries 中 125 条无 error、无 HTTP 5xx、无 timeout/conflict/rollback action，4 条消息全部到达预期收件人，Realtime、refresh、reconnect、goodbye、最终 state 均通过。只读 DB 核对确认两房两 session 均 `completed`，5 条新建 `room_members` 全部 `status=exited` 且 `exited_at` 非空，`active_member_rows=0`；`NEW GHOST=0`、`NEW ACTIVE RESIDUE=0`、`NEW DUPLICATE=0`。Supabase Observability 窗口 `06:15:27–06:25:27Z` 的 DB CPU peak=`11%`，但 baseline/final、rollback/reservation counters 仍未取得。
- **Latest accelerated checkpoint:** `0df1454` 将 `/api/auth/login` 应用层 source-IP burst 从 `30/15min` 提高为可配置默认 `300/15min`，保留单账号 `10/15min` 与 Auth 安全保护；`ff553b4` 移除未来 stage 预登录，`4d8a5b2` 限制 Runner state-read/Realtime I/O 并发。修正后的 `breakpoint-authfix-rerun40-20260825` 为 `40/40` app login、`40/40` 普通 Supabase sign-in，未出现 5xx/429/业务请求错误，但 120 秒内只形成 3 个 pair、2 个完整 group，未达到 36 个 active sessions，`40-USER STATEFUL=INCONCLUSIVE`，Matching degradation observed；未进入 Realtime/Chat/Goodbye/Feedback。该批次已通过普通 API 收敛，`finalActive=0`。用户补充 Supabase Observability 窗口 `07:08:37–07:18:37Z` 的 DB CPU peak=`16%`；baseline/final、RAM/connections 仍缺失，不得据此宣称完整数据库容量已验证。
- **Production status:** `FIX_IN_PROGRESS`；runtime health version `0df1454`，Git deployed application commit `0df1454`。Production business data manually modified `NO`；此前 migration executed `YES`（forward-only terminal member transition）；本轮仅部署应用限流配置代码，未执行 migration、未修改 Production schema；历史 migration history modified `NO`。旧 completed-room active residues未做 raw SQL 清理，继续冻结为历史基线。
