...
|---|---|---|---|---|
| `20260828120000_room_goodbye_reliability.sql` | `CONFIRMED_EXECUTED` | `NOT_RECORDED` | `997d5896eed8a6688b7370491c69b5a08a0f37e536be0099544731fe6f1478e5` | 已按明确授权在 Production Supabase SQL Editor 成功执行一次。Room-first 的 `ready` Session 现在可通过正式 Goodbye/finalize lifecycle 收敛；保留 `playing` 支持、幂等完成、成员权限与 terminal trigger 行为。无历史数据清理、无 migration replay/repair；不得直接重复执行。 |
| `20260825160000_terminalize_room_members_on_terminal.sql` | `CONFIRMED_EXECUTED` | `NOT_RECORDED` | `1b834906140b24165b6c37fc8274eab04191b4a63c9fd0c64866b46cba08c13a` | 已按明确授权在 Production Supabase SQL Editor 执行一次。Session/Room 进入 terminal 时，将仍为 `active` 的 `room_members` 更新为 `exited` 并写入 `exited_at`；session/room triggers 与 helper 已只读确认。无历史 backfill、无删除、未 replay/repair migration history；不得直接重复执行。 |
