...
访问 `https://jiyuan.online/ops`，输入服务器环境变量中的 `OPS_PASSWORD`。页面不在
公开导航中，数据接口同时要求服务端令牌或 12 小时有效的 HttpOnly 运营会话。

看板提供 7、14、30、90 天切换，展示实时人数、核心数字、完整漏斗、每日趋势、
最近错误和正式环境状态。退出后会立即清除运营会话。修改密码只需更新服务器
`.env.production` 的 `OPS_PASSWORD` 并重新创建应用容器，不需要改数据库。
