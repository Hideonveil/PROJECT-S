...
export function mePage(state) {
  ...
  return homeShell(state, `<section class="player-profile-workspace">
    ...
    <section class="player-profile-hero"><div class="player-profile-identity"><div class="player-profile-avatar">${avatarWrap(user.avatarKey, 106, user.online)}<i aria-label="在线"></i></div><div><span>JIYUAN PLAYER</span><h2>${esc(user.nickname || "未命名玩家")}</h2><p>${esc(user.handle || user.friendCode || "PLAYER ID")}</p></div></div><div class="player-profile-status"><i></i><span>当前状态</span><b>${user.online ? "在线" : "离线"}</b></div><div class="player-profile-tape" aria-hidden="true"><span>PLAYER PROFILE / NEVER PLAY ALONE / PLAYER PROFILE /</span></div></section>
