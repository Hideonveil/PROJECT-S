    ...
    `<div class="prism-page prism-friends">
      ...
      ${incoming.length ? `<section class="prism-section">
        ...
        <div class="friends-list">${incoming.map(({ user }) => `<div class="friend-row">
          <div class="friend-main">${avatarWrap(user.avatarKey, 52, user.online)}<div class="friend-meta"><div class="friend-name">${esc(user.name || user.nickname)}</div><div class="friend-last">想添加你为“机”缘好友</div></div></div>
        ...
        ${
          friends.length === 0
            ? `<div class="empty-state">
                ${icon("users", 30)}
                <strong>还没有朋友</strong>
                <span>可以在上方输入好友代码，或在匹配房间中把对方添加为“机”缘好友。</span>
              </div>`
