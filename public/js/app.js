...
document.addEventListener("click", (event) => {
  ...
  const actions = {
    ...
    "submit-forgot-password": () => authController.submitForgotPassword(),
    "submit-password-reset": () => authController.submitPasswordReset(),
    ...
    "auth-submit": () => authController.submitAuth(),
    "verify-email": () => authController.submitEmailVerification(),
    "resend-verification": () => authController.resendEmailVerification(),
    "cancel-email-verification": () => authController.cancelEmailVerification(),
...
document.addEventListener("submit", (event) => {
  event.preventDefault();
  if (event.target.matches('[data-form="auth"]')) authController.submitAuth();
  if (event.target.matches('[data-form="auth-verify"]')) authController.submitEmailVerification();
  if (event.target.matches('[data-form="auth-forgot"]')) authController.submitForgotPassword();
  if (event.target.matches('[data-form="auth-reset"]')) authController.submitPasswordReset();
});
